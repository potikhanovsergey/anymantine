import { Group, Stack, Radio, Text } from "@mantine/core"
...
const radioComponentDocsProps: Omit<ComponentDocsProps, "preview"> = {
  ...
  examples: [
        ...
        <Group>
          {["with label", "alone"].map((variant) => (
            <Stack key={variant}>
              ...
              <Radio value="kiwi" label={variant === "with label" && "Kiwi"} />{" "}
            </Stack>
          ))}
        </Group>
      ),
    ...
    },
    {
      ...
      children: (
        <Group>
          {["not selected", "selected"].map((variant) => (
            <Radio key={variant} disabled label="Kiwi" readOnly checked={variant === "selected"} />
          ))}
        </Group>
      ),
    },
